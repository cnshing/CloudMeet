...
<PageShell>
	<PageHeader title="Edit Event Type" backHref="/dashboard" backLabel="Dashboard" />
			...
			<form method="POST" use:enhance={handleSubmit}>
				<div class="space-y-6">
					...
					<div>
						...
						<p class="text-xs text-subtle-foreground mb-3">This image will be displayed at the top of your booking page </p>
						{#if coverImage}
					...
					{#if hasGoogle || hasOutlook}
						<div class="border-t border-border pt-6">
							...
							<div class="mb-4 p-3 bg-background rounded-lg text-sm border border-border">
								<p class="text-muted-foreground mb-1"><span class="font-medium">Check availability from:</span> {getAvailabilityLabel(getGlobalAvailability())}</p>
								<p class="text-muted-foreground"><span class="font-medium">Send invite via:</span> {getInviteLabel(getGlobalInviteCalendar())}</p>
