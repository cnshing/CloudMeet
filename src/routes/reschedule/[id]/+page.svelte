...
<div class="min-h-screen bg-background flex flex-col items-center justify-center p-4">
	...
	{:else}
		...
		<div class="bg-surface rounded-2xl shadow-lg overflow-hidden flex transition-all duration-300 ease-in-out" style="width: {selectedDate ? '920px' : '650px'}">
			...
			<div class="flex-1 p-6">
				{#if rescheduleError}
					<div class="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 mb-6 max-w-2xl">{rescheduleError}</div>
				{/if}
								...
								<div class="space-y-2 overflow-y-auto flex-1 pr-2 pb-2 scrollbar-thin">
									{#each availableSlots as slot}
										{#if selectedSlot === slot}
											<button type="button" class="w-full py-2.5 px-3 border-2 border-foreground bg-foreground text-background rounded-lg text-sm font-semibold">
