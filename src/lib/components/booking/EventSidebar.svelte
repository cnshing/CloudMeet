	...
	<div class="flex-1 p-6">
		...
		{#if eventType?.description}
			<div class="mt-6 pt-6 border-t border-border">
				<div class="text-sm text-muted-foreground prose prose-sm max-w-none [&_p]:my-2 [&_ul]:my-2 [&_ol]:my-2 [&_li]:my-1">
		...
		{#if selectedDate && selectedSlot}
			<div class="mt-6 pt-6 border-t border-border">
				<div class="flex items-center gap-3 text-sm">
					...
					<div>
						<p class="font-medium text-foreground">{formatTime(selectedSlot.start)} - {formatTime(selectedSlot.end)}</p>
