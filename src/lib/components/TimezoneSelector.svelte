...
<div class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] max-w-md bg-surface border border-border rounded-xl shadow-2xl z-50 max-h-[80vh] flex flex-col overflow-hidden">
	<!-- Search -->
	<div class="p-3 border-b border-border">
		<div class="relative">
			<svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-subtle-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	<div class="px-4 py-2 border-b border-border bg-background">
		<span class="text-xs font-semibold text-subtle-foreground uppercase tracking-wide">Time zone</span>
	</div>
	...
	<div class="overflow-y-auto flex-1" role="listbox" aria-label="Time zone">
		{#each Object.entries(filteredGroups()) as [group, zones]}
			<div role="group" aria-labelledby="group-{group}">
				<div id="group-{group}" class="px-4 py-2 text-xs font-semibold text-subtle-foreground uppercase tracking-wide bg-background sticky top-0">
					{group}
				</div>
				{#each zones as tz}
					...
					<button
						...
						class="w-full px-4 py-2.5 flex items-center justify-between text-left text-sm hover:bg-surface-2 transition-colors {isSelected ? 'font-medium bg-accent-subtle text-accent' : 'text-muted-foreground'}"
					>
						...
						<span class="text-subtle-foreground tabular-nums">{getCurrentTime(tz.value, false)}</span>
					</button>
				{/each}
			</div>
		{/each}
